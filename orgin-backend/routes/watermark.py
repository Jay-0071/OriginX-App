...
def lookup_signature(image_bytes: bytes, db: Session) -> dict:
    ...
    if recovered_sig:
        ...
        # Match against all registered users using cryptographic HMAC
        # We must recompute the expected HMAC for each user using their secret key and this image's p_hash
        try:
            p_hash_for_hmac = compute_perceptual_hash(image_bytes)
        except:
             p_hash_for_hmac = "" # Fallback if pHash generation fails
             
        records = db.query(OriginRegistry).all()
        for r in records:
            # Recompute what the HMAC payload *should* be for this user and this image
            if r.secret_key:
                from services.watermark_service import build_cryptographic_payload
                expected_payload = build_cryptographic_payload(r.secret_key, p_hash_for_hmac)
                expected_sig = expected_payload[2:].hex()
            else:
                 # Legacy fallback for old records
                 expected_sig = r.signature_hash if r.signature_hash else compute_signature(r.watermark_seed)

            if match_signature_tolerant(recovered_sig, expected_sig, max_hamming=2):
                hamming = bin(int(recovered_sig, 16) ^ int(expected_sig, 16)).count('1') if recovered_sig != expected_sig else 0
                print(f"[LOOKUP] ✓ STRATEGY 3/4 — EMBEDDED WATERMARK MATCH → @{r.user_handle} (hamming={hamming})")
                return _success(r.user_handle, r, "CRYPTOGRAPHIC_WATERMARK",
                                f"cryptographically bound watermark signature matches @{r.user_handle}")
        ...
        return {
            ...
            "message": "Watermark payload extracted but does not match any registered identity or image binding",
        }
...
async def embed(
    ...
    Now secured with user-specific passwords for cryptographic bound generation.
    """
    if not file:
        raise HTTPException(status_code=400, detail={"message": "No image file provided."})
    if not user_handle:
        raise HTTPException(status_code=400, detail={"message": "User handle is required."})
    if not password:
        raise HTTPException(status_code=400, detail={"message": "Password is required for secure cryptographic embedding."})
    ...
    raw = await file.read()
    print(f"\n{'='*60}")
    print(f"[EMBED] Received {len(raw)} bytes for @{user_handle} (secure, consent=accepted)")
    normalized = _normalize_image(raw)
    ...
    # ── Verify user password for cryptographic payload ──
    try:
        if not bcrypt.checkpw(password.encode('utf-8'), record.password_hash.encode('utf-8')):
            raise HTTPException(
                status_code=401,
                detail={"message": "Invalid password for this user handle."}
            )
    except Exception as e:
        print(f"[EMBED] Password verification failed: {e}")
        raise HTTPException(
            status_code=401,
            detail={"message": "Invalid password or corrupted authentication record."}
        )
    ...
    # Create the cryptographically bound watermark using the user's Secret Key
    watermarked = embed_watermark(normalized, record.secret_key)
