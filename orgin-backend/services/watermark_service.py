...
SIGNATURE_BYTES = 6              # 6 bytes of HMAC digest
...
def compute_signature(seed: str) -> str:
    """Compute the canonical 12-char hex signature from a watermark seed or secret key."""
    digest = hashlib.sha256(seed.encode("utf-8")).digest()[:SIGNATURE_BYTES]
    return digest.hex()  # 12-char hex string


def build_cryptographic_payload(secret_key: str, p_hash: str) -> bytes:
    """Build the 8-byte binary payload for DwtDct embedding using HMAC-SHA256.
    Format: b'OX' + HMAC(key=secret_key, msg=p_hash)[:6] = 8 bytes exactly.
    This mathematically binds the watermark to the specific image."""
    hmac_digest = hmac.new(
        key=secret_key.encode('utf-8'),
        msg=p_hash.encode('utf-8'),
        digestmod=hashlib.sha256
    ).digest()[:SIGNATURE_BYTES]
    
    payload = PAYLOAD_HEADER + hmac_digest
...
def embed_watermark(image_bytes: bytes, secret_key: str) -> bytes:
    """Embed watermark using DwtDct (primary) + LSB (secondary).
    Both layers embed the cryptographically bound HMAC signature."""
    # Compute the image's perceptual hash to mathematically bind the payload to this image
    p_hash = compute_perceptual_hash(image_bytes)
    
    # Generate the HMAC payload
    payload = build_cryptographic_payload(secret_key, p_hash)
    
    # The signature is just the hex representation of the HMAC digest
    sig_hex = payload[2:].hex()
    
    print(f"  [WM-EMBED] Image pHash: {p_hash}")
    print(f"  [WM-EMBED] Generated HMAC Signature: {sig_hex}")
