...
def test_embed_workflow():
    ...
    # First register the test user with a password
    reg_data = {
        "user_handle": "rom",
        "password": "securepassword123",
        "watermark_seed": "originx_test_seed"
    }
    try:
        # Ignore 400 if user exists, just try to register
        requests.post(f"{BASE_URL}/api/consent/register", json=reg_data)
    except:
        pass
        
    # Embed watermark for existing user 'rom' with their password
